...
This directory contains comprehensive Redis configuration files and scripts for Wrangler development and testing, supporting both cluster and single-node setups with optional TLS encryption.
...
REDIS_CA=/path/to/Wrangler/redis-config/certs/ca-cert.pem
```
...
ls -la /path/to/Wrangler/redis-config/certs/ca-cert.pem

# Test Wrangler Redis configuration
cd /path/to/Wrangler
npm run backend
