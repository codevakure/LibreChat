 ...
 * Priority:
 * 1. PLEACH_LOG_DIR environment variable
 * 2. If running within Pleach monorepo (when cwd ends with /api), use api/logs
...
export const getLogDirectory = (): string => {
  if (process.env.PLEACH_LOG_DIR) {
    return process.env.PLEACH_LOG_DIR;
  }
  ...
  // For Pleach project structure, use api/logs
  // For external consumers, they should set PLEACH_LOG_DIR
  if (cwd.includes('Pleach')) {
    return apiLogsPath;
  }
