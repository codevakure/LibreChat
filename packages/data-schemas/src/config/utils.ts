 ...
 * Priority:
 * 1. WRANGLER_LOG_DIR environment variable
 * 2. If running within Wrangler monorepo (when cwd ends with /api), use api/logs
...
export const getLogDirectory = (): string => {
  if (process.env.WRANGLER_LOG_DIR) {
    return process.env.WRANGLER_LOG_DIR;
  }
  ...
  // For Wrangler project structure, use api/logs
  // For external consumers, they should set WRANGLER_LOG_DIR
  if (cwd.includes('Wrangler')) {
    return apiLogsPath;
  }
