...
import type { IRole } from '@wrangler/data-schemas';
...
function hasExplicitConfig(
  ...
  switch (permissionType) {
    ...
    case PermissionTypes.CHARTS:
      return interfaceConfig?.charts !== undefined;
    default:
      return false;
  }
}
...
export async function updateInterfacePermissions({
  ...
  // 1. Explicit user configuration (from wrangler.yaml)
  ...
  for (const roleName of [SystemRoles.USER, SystemRoles.ADMIN]) {
    ...
    const addPermissionIfNeeded = (
      ...
      const permTypeExists = existingPermissions?.[permType as keyof typeof existingPermissions];
    ...
    const allPermissions: Partial<Record<PermissionTypes, Record<string, boolean | undefined>>> = {
      ...
      [PermissionTypes.CHARTS]: {
        [Permissions.USE]: getPermissionValue(
          loadedInterface.charts,
          defaultPerms[PermissionTypes.CHARTS]?.[Permissions.USE],
          defaults.charts,
        ),
      },
    };
