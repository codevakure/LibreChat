...
describe('Environment Variable Extraction (MCP)', () => {
  ...
  describe('processMCPEnv', () => {
    ...
    it('should ensure different users with same starting config get separate values', () => {
      ...
      expect(baseConfig.headers?.['User-Id']).toBe('{{PLEACH_USER_ID}}');
    ...
    it('should process dynamic user fields in headers', () => {
      ...
      const options: MCPOptions = {
        ...
        url: 'https://example.com',
        headers: {
          'User-Email': '{{PLEACH_USER_EMAIL}}',
          'User-Name': '{{PLEACH_USER_USERNAME}}',
          OpenID: '{{PLEACH_USER_OPENIDID}}',
          'Google-ID': '{{PLEACH_USER_GOOGLEID}}',
          'Email-Verified': '{{PLEACH_USER_EMAILVERIFIED}}',
          'User-Role': '{{PLEACH_USER_ROLE}}',
          'Content-Type': 'application/json',
        },
      };
    ...
    it('should process user fields in env variables', () => {
      ...
      const options: MCPOptions = {
        ...
        args: ['server.js'],
        env: {
          USER_EMAIL: '{{PLEACH_USER_EMAIL}}',
          LDAP_ID: '{{PLEACH_USER_LDAPID}}',
          API_KEY: '${TEST_API_KEY}',
        },
      };
    ...
    it('should process user fields in URL', () => {
      ...
      const options: MCPOptions = {
        type: 'sse',
        url: 'https://example.com/api/{{PLEACH_USER_USERNAME}}/stream',
      };
    ...
    it('should handle boolean user fields', () => {
      ...
      const options: MCPOptions = {
        ...
        headers: {
          'Email-Verified': '{{PLEACH_USER_EMAILVERIFIED}}',
          'Two-Factor': '{{PLEACH_USER_TWOFACTORENABLED}}',
          'Terms-Accepted': '{{PLEACH_USER_TERMSACCEPTED}}',
        },
      };
    ...
    it('should not process sensitive fields like password', () => {
      ...
      const options: MCPOptions = {
        ...
        headers: {
          'User-Email': '{{PLEACH_USER_EMAIL}}',
          'User-Password': '{{PLEACH_USER_PASSWORD}}', // This should not be processed
        },
      };
      ...
      expect('headers' in result && result.headers).toEqual({
        'User-Email': 'test@example.com',
        'User-Password': '{{PLEACH_USER_PASSWORD}}', // Unchanged
      });
    });
    ...
    it('should handle multiple occurrences of the same placeholder', () => {
      ...
      const options: MCPOptions = {
        ...
        url: 'https://example.com',
        headers: {
          'Primary-Email': '{{PLEACH_USER_EMAIL}}',
          'Secondary-Email': '{{PLEACH_USER_EMAIL}}',
          'Backup-Email': '{{PLEACH_USER_EMAIL}}',
        },
      };
    ...
    it('should support both id and _id properties for PLEACH_USER_ID', () => {
      ...
      expect('headers' in result2 && result2.headers?.['User-Id']).toBe('{{PLEACH_USER_ID}}');
    ...
    it('should process customUserVars in URL field', () => {
      ...
      const options: MCPOptions = {
        type: 'websocket',
        url: 'wss://example.com/{{TENANT_ID}}/api/{{API_VERSION}}?user={{PLEACH_USER_ID}}&key=${TEST_API_KEY}',
      };
    ...
    it('should prioritize customUserVars over user fields and system env vars if placeholders are the same (though not recommended)', () => {
      ...
      process.env.PLEACH_USER_EMAIL = 'system-email-should-be-overridden';
      const user = createTestUser({ email: 'user-email-should-be-overridden' });
      const customUserVars = {
        PLEACH_USER_EMAIL: 'custom-email-wins',
      };
      const options: MCPOptions = {
        type: 'sse',
        url: 'https://example.com/api',
        headers: {
          'Test-Email': '{{PLEACH_USER_EMAIL}}', // Placeholder that could match custom, user, or system
        },
      };
      ...
      // Clean up env var
      delete process.env.PLEACH_USER_EMAIL;
    });
    ...
    it('should handle placeholders with no matching customUserVars (falling back to user/system vars)', () => {
      ...
      const options: MCPOptions = {
        ...
        url: 'https://example.com/api',
        headers: {
          'User-Email-Header': '{{PLEACH_USER_EMAIL}}', // Should use user.email
    ...
    it('should correctly process a mix of all variable types', () => {
      ...
      const obj = {
        type: 'streamable-http' as const,
        url: 'https://{{CUSTOM_ENDPOINT_ID}}.example.com/users/{{PLEACH_USER_USERNAME}}',
        headers: {
        ...
        env: {
          ...
          USER_HOME_DIR: '/home/{{PLEACH_USER_USERNAME}}',
    ...
    it('should process GitHub MCP server configuration with PAT_TOKEN placeholder', () => {
      ...
      // Simulate the GitHub MCP server configuration from pleach.yaml
      const options: MCPOptions = {
        type: 'streamable-http',
