import { extractEnvVariable } from 'pleach-data-provider';
import type { TUser, MCPOptions } from 'pleach-data-provider';
...
function processUserPlaceholders(value: string, user?: TUser): string {
  ...
  for (const field of ALLOWED_USER_FIELDS) {
    const placeholder = `{{PLEACH_USER_${field.toUpperCase()}}}`;
 ...
 * Recognized placeholders: `{{PLEACH_BODY_<FIELD>}}` where `<FIELD>` ∈ ALLOWED_BODY_FIELDS.
...
function processBodyPlaceholders(value: string, body: RequestBody): string {
  for (const field of ALLOWED_BODY_FIELDS) {
    const placeholder = `{{PLEACH_BODY_${field.toUpperCase()}}}`;
    if (!value.includes(placeholder)) {
      continue;
    }
...
function processSingleValue({
  ...
  // 2. Replace user field placeholders (e.g., {{PLEACH_USER_EMAIL}}, {{PLEACH_USER_ID}})
  value = processUserPlaceholders(value, user);

  // 3. Replace body field placeholders (e.g., {{PLEACH_BODY_CONVERSATIONID}}, {{PLEACH_BODY_PARENTMESSAGEID}})
  if (body) {
