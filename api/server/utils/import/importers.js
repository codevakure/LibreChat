...
const { EModelEndpoint, Constants, openAISettings, CacheKeys } = require('pleach-data-provider');
...
function getImporter(jsonData) {
  ...
  if (jsonData.conversationId && (jsonData.messagesTree || jsonData.messages)) {
    logger.info('Importing Pleach conversation');
    return importPleachConvo;
  }
...
/**
 * Imports a Pleach conversation from JSON.
 *
 ...
 * @returns {Promise<void>} - A promise that resolves when the import is complete.
 */
async function importPleachConvo(
  jsonData,
  requestUserId,
  builderFactory = createImportBatchBuilder,
) {
  try {
    ...
    if (jsonData.recursive) {
      ...
      throw new Error('Invalid Pleach file format');
    }
    ...
    logger.error(`user: ${requestUserId} | Error creating conversation from Pleach file`, error);
  }
}
