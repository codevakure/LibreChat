...
const { EModelEndpoint, Constants, openAISettings, CacheKeys } = require('wrangler-data-provider');
...
function getImporter(jsonData) {
  ...
  if (jsonData.conversationId && (jsonData.messagesTree || jsonData.messages)) {
    logger.info('Importing Wrangler conversation');
    return importWranglerConvo;
  }
...
/**
 * Imports a Wrangler conversation from JSON.
 *
 ...
 * @returns {Promise<void>} - A promise that resolves when the import is complete.
 */
async function importWranglerConvo(
  jsonData,
  requestUserId,
  builderFactory = createImportBatchBuilder,
) {
  try {
    ...
    if (jsonData.recursive) {
      ...
      throw new Error('Invalid Wrangler file format');
    }
    ...
    logger.error(`user: ${requestUserId} | Error creating conversation from Wrangler file`, error);
  }
}
