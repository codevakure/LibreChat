# Wrangler Multi-Database Support Plan

## Overview

This document outlines the plan to support both MongoDB and PostgreSQL in Wrangler, allowing users to choose their preferred database during initial setup. Users will select either MongoDB OR PostgreSQL at installation time - there is no migration between databases or dual-database operation.

## Current Architecture

Wrangler currently uses:
...
After deep analysis of the Wrangler codebase, supporting both MongoDB and PostgreSQL end-to-end presents **significant technical challenges** that need honest evaluation:
...
You're absolutely correct! If we're doing the substantial work to make Wrangler work with PostgreSQL, we're essentially solving all the hard problems already. At that point, **keeping MongoDB support is actually logical** because:
...
MONGO_URI=mongodb://127.0.0.1:27017/Wrangler

# PostgreSQL Configuration (new)
POSTGRES_HOST=localhost
POSTGRES_PORT=5432
POSTGRES_DATABASE=wrangler
POSTGRES_USERNAME=wrangler_user
...
services:
  api:
    ...
    depends_on:
      - database
      - rag_api
    image: ghcr.io/danny-avila/wrangler-dev:latest
    environment:
      - HOST=0.0.0.0
      - DATABASE_TYPE=${DATABASE_TYPE:-mongodb}
      - MONGO_URI=mongodb://mongodb:27017/Wrangler
...
Wrangler supports two database options:

### Option 1: MongoDB (Default)
```bash
# Set environment variable
DATABASE_TYPE=mongodb
MONGO_URI=mongodb://localhost:27017/Wrangler
